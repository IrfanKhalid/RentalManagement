...
export class MemberRegistrationService {
    ...
    public SavePlot(memberModel: PlotModel,id:number)
    {
        
        var SaveUrl = environment.apiEndpoint +"/api/RegisterMember/AddPlot";
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.token}`);
        return this.http.post<any>(SaveUrl, memberModel, { headers: headers })
            .pipe(
    ...
    // Get All Plot by MemberId
    GetAllPlotsByMemberId<T>(MemberId) {
        let headers = new HttpHeaders({ 'Content-Type': 'application/json' });
        headers = headers.append('Authorization', 'Bearer ' + `${this.token}`);
        let Url = environment.apiEndpoint +"/api/MemberPlot/GetAllPlot/"+MemberId;
        const mergedUrl = `${Url}` +
            `?page=${this.paginationService.page}&pageCount=${this.paginationService.pageCount}`;
