...
namespace WebGYM.Controllers
{
    ...
    public class RegisterMemberController : ControllerBase
    {
            ...
            if (ModelState.IsValid)
            {
                var storedMemberid =
                    _memberRegistration.CheckNameExitsforUpdate(member.MemberFName, member.MemberLName,
                        member.MemberMName);
                if (storedMemberid == member.Id || storedMemberid == 0)
                {
                    var automember = AutoMapper.Mapper.Map<MemberRegistration>(member);
        ...
        [HttpPost("AddPlot")]
        public HttpResponseMessage PostAddPlot([FromBody] AttachPlotViewModel member)
        {
            try
            {
                if (ModelState.IsValid)
                {
                    ...
                    //{
                    var userId = this.User.FindFirstValue(ClaimTypes.Name);
                    var automember = AutoMapper.Mapper.Map<AttahPlot>(member);
                    //automember.Createdby = Convert.ToInt32(userId);

                    var result = _memberRegistration.InsertAttachPlot(automember);
                    if (result > 0)
                    {
                        var response = new HttpResponseMessage()
                        {
